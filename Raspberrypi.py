...
class ContourWithData():
  ...
  def checkIfContourIsValid(self, height, width):  # this is oversimplified, for a production grade program
    MAX_CONTOUR_AREA = height * width * 1
...
def solution(counter, marker, distance):
  ...
  f = open('{0}.txt'.format(date), "a+")
  f.write("marker = {0}, charcter = {1}, colour = {2}, time it was taken = {3} \r\n".format(marker, character(counter, marker, distance) , colour(counter, marker,distance), time))
  f.close() 

  if Static_Test:
    with open('results.csv', 'a') as csvfile: # for testing purposes
      filewriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
      filewriter.writerow([str(marker), str(character(counter, marker, distance)), str(colour(counter, marker, distance)), str(distance)])
  ...
  print("ground marker {0} recognized".format(marker))
  counter = 0
  marker = marker + 1
...
def detection():
  ...
  time.sleep(1)  # allows the camera to start-up
  print('Camera on')
  ...
  for image in camera.capture_continuous(cap, format="bgr", use_video_port=True):
    if counter == 0 or start - end < 5:
      if counter < 7:
        ...
        edged = cv2.Canny(blurred, 10, 10)  # the lower the value the more detailed it would be
        ...
        for c in contours:
          ...
          if 4 <= len(approx) <= 6:
            ...
            keepAspectRatio = 0.75 <= aspectRatio <= 1.25
            if keepDims and keepSolidity and keepAspectRatio:  # checks if the values are true
              ...
              if square == 2:
                ...
                in_gray = cv2.cvtColor(HSVCropp, cv2.COLOR_BGR2GRAY)
                in_blurred = cv2.GaussianBlur(in_gray, (5, 5), 0)
                in_edged = cv2.Canny(in_blurred, 10, 10)

                (in_contours, _) = cv2.findContours(in_edged.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

                for in_c in in_contours:
                  in_peri = cv2.arcLength(in_c, True)
                  in_approx = cv2.approxPolyDP(in_c, 0.01 * in_peri, True)
                  if 4 <= len(in_approx) <= 6:
                    (in_x, in_y, in_w, in_h) = cv2.boundingRect(in_approx)
                    in_aspectRatio =in_w / float(in_h)
                    in_area = cv2.contourArea(in_c)
                    in_hullArea = cv2.contourArea(cv2.convexHull(in_c))
                    in_solidity = in_area / float(in_hullArea)
                    in_keepDims = in_w > 25 and in_h > 25
                    in_keepSolidity = in_solidity > 0.9
                    in_keepAspectRatio = 0.8 <= in_aspectRatio <= 1.2
                    if in_keepDims and in_keepSolidity and in_keepAspectRatio:
                      color = HSVCropp[in_y: in_y + in_h, in_x: in_x + in_w]
                    else:
                      break
                  else:
                    break
              ...
              if Different_Result:
                ...
                cv2.imwrite('/home/pi/{0}/colour{1}_{2}.png'.format(marker_name, marker, counter), color)
                cv2.imwrite('/home/pi/{0}/orginal{1}_{2}.png'.format(marker_name, marker, counter), frame)


              if Static_Test:
                # testing purposes
                distance = input("distace this was taken")
                if not os.path.exists(distance):
                  os.makedirs(distance)
                cv2.imwrite('/home/pi/{0}/result{1}_{2}.png'.format(distance, marker, counter), color)
                cv2.imwrite('/home/pi/{0}/orginal{1}_{2}.png'.format(distance, marker, counter), frame)
                cv2.imwrite('/home/pi/{0}/captured{1}_{2}.png'.format(distance, marker, counter), roi)
              else:
                distance = 0
...
def character(counter, marker, distance):
  ...
  for i in range(1, counter + 1):
   try:
      ...
      img = cv2.imread("colour{0}.png".format(i))
      ...
      threshkernal = np.ones((3,3), np.uint8)
      opening =  cv2.morphologyEx(imgThresh, cv2.MORPH_OPEN, threshkernal, iterations=1)
      eroding = cv2.erode(opening,kernel, iterations=1)
      dilating = cv2.dilate(eroding,kernel,iterations=1)

      imgThreshCopy = dilating.copy()        # make a copy of the thresh image, this in necessary b/c findContours modifies the image

      if Different_Result:
        marker_name = "marker={0}".format(marker)
        cv2.imwrite('/home/pi/{0}/contour{1}_{2}.png'.format(marker_name, marker, i), imgThreshCopy)
       # cv2.imwrite('/home/pi/{0}/gauss{1}_{2}.png'.format(marker_name, marker, i), gauss)
       # cv2.imwrite('/home/pi/{0}/gray{1}_{2}.png'.format(marker_name, marker, i), gray)
       # cv2.imwrite('/home/pi/{0}/img{1}_{2}.png'.format(marker_name, marker, i), img)
       # cv2.imwrite('/home/pi/{0}/orgresize{1}_{2}.png'.format(marker_name, marker, i), resize)


      if Static_Test:
        cv2.imwrite('/home/pi/{0}/character{1}_{2}.png'.format(distance, marker, i), imgThreshCopy)
     ...
     # print(validContoursWithData)
      ...
      for contourWithData in validContoursWithData:            # for each contour
         # new = cv2.cvtColor(cv2.rectangle(roi,  # draw rectangle on original testing image
          ...
          imgROI = dilating[contourWithData.intRectY + 1: contourWithData.intRectY + contourWithData.intRectHeight - 1,     # crop char out of threshold image
          ...
          if Different_Result:
            marker_name = "marker={0}".format(marker)
            cv2.imwrite('/home/pi/{0}/resize{1}_{2}.png'.format(marker_name, marker, i), imgROIResized)
...
def colour(counter, marker, distance):
  ...
  for k in range(1, counter + 1):
      ...
      if Static_Test:
        cv2.imwrite('/home/pi/{0}/colour{1}_{2}.png'.format(distance, marker, k), denoised)

      if Different_Result:
        marker_name= "marker={0}".format(marker)
        cv2.imwrite('/home/pi/{0}/colour{1}_{2}.png'.format(marker_name, marker, k), denoised)
...
def main():
  if GPS:
    ...
    vehicle = connect('127.0.0.1:6969', wait_ready=True, baud=921600)
